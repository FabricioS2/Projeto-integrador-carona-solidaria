...
# views.py - Modificar a view chat
...
#     # Verificar se o usuário está em alguma carona ativa
#     carona_ativa = None
#     is_motorista = False
#     erro = None
...
#         motorista=usuario,
#         status__in=['Agendada', 'Em Andamento']  # Caronas ativas
#     )
    
#     for carona in caronas_motorista:
#         # Verificar se tem pelo menos uma pessoa aceita
#         if SolicitacaoCarona.objects.filter(carona=carona, status='Aceita').exists():
#             carona_ativa = carona
#             is_motorista = True
#             break
    
#     # Verificar como passageiro
#     if not carona_ativa:
#         solicitacoes_aceitas = SolicitacaoCarona.objects.filter(
#             passageiro=usuario,
#             status='Aceita',
#             carona__status__in=['Agendada', 'Em Andamento']
#         )
...
#     if carona_ativa:
#         # Buscar mensagens da carona ativa
#         mensagens = MensagemChat.objects.filter(
#             carona=carona_ativa,
#             carona_ativa=True
#         ).select_related('usuario').order_by('data_envio')
        
#         # Marcar mensagens antigas como inativas se a carona foi finalizada
#         if carona_ativa.status == 'Finalizada':
#             # Esconder o chat
#             erro = "Esta carona foi finalizada. O chat não está mais disponível."
#             # Atualizar todas as mensagens para inativas
#             mensagens.update(carona_ativa=False)
#             mensagens = []
#     else:
#         erro = "Você precisa estar em uma carona ativa para acessar o chat."
#         mensagens = []
    
#     return render(request, 'usuario_app/chat.html', {
#         'usuario': usuario,
#         'mensagens': mensagens,
#         'carona_ativa': carona_ativa,
...
# views.py - Adicione no início com os outros imports
...
def finalizar_carona(request, carona_id):
    ...
    if carona.status == 'Em Andamento':
        ...
        # Marcar todas as mensagens desta carona como inativas
        MensagemChat.objects.filter(carona=carona).update(carona_ativa=False)
...
def remover_carona(request, carona_id):
    ...
    # Marcar carona como removida em vez de deletar
...
from django.shortcuts import render, redirect, get_object_or_404
from .models import Usuario, Carona, SolicitacaoCarona, Veiculo
...
def solicitar_carona(request):
    ...
    # (Apenas solicitações de caronas NÃO FINALIZADAS são consideradas ativas)
...
from django.db.models import Q
from .models import Usuario, Carona, SolicitacaoCarona, Veiculo, Notificacao
import json
