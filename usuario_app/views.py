...
def solicitar_carona(request):
    ...
    try:
        ...
        # Se usuário não existe, limpar sessão e redirecionar para login
    ...
    # Aplicar filtro de data - CORREÇÃO IMPORTANTE
    if data_filtro:
        try:
            # Converter string para date object
            data_obj = datetime.strptime(data_filtro, '%Y-%m-%d').date()
            # Filtrar por data (ignorando a hora)
            caronas_query = caronas_query.filter(
            ...
            # Se houver erro na data, não aplicar filtro
            pass
    ...
    # **REMOVIDO O FILTRO DE DATA FUTURA - MOSTRAR TODAS AS AGENDADAS**
    # caronas_query = caronas_query.filter(horario_e_data__gte=timezone.now())
    
    # Ordenar por data/horário mais próximo (mais antigas primeiro)
